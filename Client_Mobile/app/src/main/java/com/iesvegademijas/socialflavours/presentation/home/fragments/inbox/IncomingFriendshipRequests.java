    ...
    public void acceptFriendship(int position) {
        if (friendshipRequestsModels!=null)
        {
            if (friendshipRequestsModels.size() > 0)
            {
                ...
                if (isNetworkAvailable()) {
                    String url = getResources().getString(R.string.main_url)+ "friendshipapi/" + friendShip.getId_friendship() + "/accept";
                    sendTask(url);
                } else {
                    showError("error.IOException");
                }
    ...
    private void deleteRequest(int position)
    {
        if (friendshipRequestsModels!=null)
        {
            if (friendshipRequestsModels.size() > 0)
            {
                ...
                if (isNetworkAvailable()) {
                    String url = getResources().getString(R.string.main_url) + "friendshipapi/" + friendShip.getId_friendship() + "/reject";
                    deleteTask(url);
                } else {
                    showError("error.IOException");
                }
